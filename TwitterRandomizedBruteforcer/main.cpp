...
#define NUMBEROFCHARS 35 // Defines how many chars are in the char array below
...
int main()
{
    ...
    int MinSize = 0;
    int MaxSize = 0;
    printf( "\nEnter the minimum character size of the password (cannot be less then 6): ");
    std::cin >> MinSize;
    ...
    printf( "\nEnter the maximum character size of the password (cannot be greater then 100): ");
    std::cin >> MaxSize;
    ...
    while(true){
        ...
        if(MaxSize != MinSize)
        PasswordSize = rand() % MaxSize + MinSize;
        else
        PasswordSize = MaxSize;
        
        for(int i = 0; i < PasswordSize; i++){
                
                int choosechar = rand() % NUMBEROFCHARS + 0;
                baseString += chars[choosechar];
